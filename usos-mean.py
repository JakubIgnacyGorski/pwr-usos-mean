...
class subject:
  def __init__(self, name, grade, ects):
    ...
    self.ects = ects
  def print_row(self):
    print(f"| {str(self.name).center(NAME_WIDTH)} | {str(self.grade).center(GRADE_WIDTH)} | {str(self.ects).center(ECTS_WIDTH)} |")
...
class semester:
  ...
  def print_semester(self):
    print(f"| {'Nazwa przedmiotu'.center(NAME_WIDTH)} | {'Ocena'.center(GRADE_WIDTH)} | {'ECTS'.center(ECTS_WIDTH)} |")
    print(f"| {'-'*NAME_WIDTH} | {'-'*GRADE_WIDTH} | {'-'*ECTS_WIDTH} |")
    for subject in self.subjects:
      subject.print_row()
    mean_text = f"{self.mean:.3f}"
    print(f"| {'Podsumowanie semestru'.rjust(NAME_WIDTH)} | {mean_text.ljust(GRADE_WIDTH)} | { str(self.ects_sum).center(ECTS_WIDTH) } |")
    print(f"| {'-'*NAME_WIDTH} | {'-'*GRADE_WIDTH} | { '-'*ECTS_WIDTH } |")
  def get_grades(self):
    grades = []
    for subject in self.subjects:
...
class years:
  ...
  def print_year(self):
    ...
    print(f"| {'Podsumowanie roku'.rjust(NAME_WIDTH)} | {mean_text.center(GRADE_WIDTH)} | {str(self.ects_sum).center(ECTS_WIDTH)} |")
  ...
  with open(pdfname, 'rb') as file:
    ...
    for page_num in range(len(reader.pages)):
      ...
      for line in lines:
          ...
          if "Przedmioty wg cykli dydaktycznych Zajęcia/godziny Ocena ECTS" in line:
          ...
          if start_reading and not line.startswith("Strona") and not line.startswith("USOSweb: Karta przebiegu studiów"):
...
parser = argparse.ArgumentParser(description="Program przyjmuje jeden obowiązkowy argument: --file")

# Dodajemy obowiązkowe argumenty
parser.add_argument("-f", "--file", required=True, type=str, help="Podaj ścieżke do pliku pdf")
